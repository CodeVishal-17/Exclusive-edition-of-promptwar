...
| **Understand** | 1 Gemini call with `ANALYSIS_SCHEMA`. The result is cached in memory for 15 minutes, keyed by a SHA-256 hash of the document plus options. Identical requests arriving together share one call. |
| **Ask** | 1 call per question with `ANSWER_SCHEMA`, including the last 8 turns. An identical question about the same document, with the same history, language and reading level, is answered from a 5-minute cache. If `answeredFromDocument` is false, citations are dropped and the UI says "Not answered by the document". An answer without citations, or with citations that can't be found, gets a visible warning. |
...
- **Storage.** Documents are never written to disk or kept in the browser. Analysis and comparison **results** (which can include short quotes) are cached **in server memory for up to 15 minutes** (`CACHE_TTL_MS`), and Ask answers for **5 minutes** (`ASK_CACHE_TTL_MS`), so repeat requests don't call the model again. Set `CACHE_ENTRIES=0` to disable caching. Cache keys are SHA-256 hashes, so no document text is used as a key, and results over 256 KB are not cached. API responses send `Cache-Control: no-store`.
...
- **Abuse limits.** These all run in memory, per instance:
  - AI requests are rate-limited per IP (`RATE_LIMIT_PER_MINUTE`, default 20).
  - At most **2 AI requests can run at once per client** and 10 per instance (`AI_CONCURRENCY_PER_CLIENT`, `AI_CONCURRENCY_TOTAL`). Extra requests get a clear `429`. The slot is taken before the upload is read, and released on success, error, timeout or disconnect.
  - Analyze and Ask accept bodies up to 10 MB; only Compare, which carries two documents, accepts 20 MB.
  - Each model call has a time limit (`GEMINI_TIMEOUT_MS`, default 60 s), and so does each whole request (`GEMINI_TOTAL_TIMEOUT_MS`, default 110 s). Conversation history length is limited too.
  - The per-IP limits do not protect the key's overall provider quota (see [Provider errors](#provider-errors-overload-vs-quota)).
- **Client IP.** Rate limits use the client IP, and `X-Forwarded-For` is trusted only for the number of proxy hops in `TRUST_PROXY`. The default is `0` (the header is ignored, so it can't be spoofed), or `1` when running on Render or Cloud Run, which each add exactly one proxy. With one trusted hop, only the address the proxy itself appended is used, so values a client prepends are ignored (tested).
- **Cross-site requests.** Browser `POST`s to the AI endpoints must come from the page's own origin, or from one listed in `ALLOWED_ORIGINS` (Render's `RENDER_EXTERNAL_URL` is included automatically). Other origins get `403` before the body is read and before they count against anyone's rate limit. Requests with no `Origin` header, such as curl or scripts, are still accepted and still rate-limited.
- **Duplicate work.** Identical requests that arrive while one is already running share a single Gemini call (single-flight). Failed or timed-out calls are never cached.
...
| `ASK_CACHE_TTL_MS` | `300000` | Lifetime of cached Ask answers |
| `AI_CONCURRENCY_PER_CLIENT` / `AI_CONCURRENCY_TOTAL` | `2` / `10` | Simultaneous AI requests per client IP, and per instance |
| `ALLOWED_ORIGINS` | — | Extra browser origins allowed to call the AI endpoints (comma-separated). The page's own origin is always allowed. |
| `PORT` | `8080` | HTTP port; the host sets it (Render uses 10000, Cloud Run 8080) |
| `TRUST_PROXY` | `0`, or `1` on Render / Cloud Run | Number of reverse-proxy hops to trust for the client IP. Never set it higher than the real number of proxies. |
